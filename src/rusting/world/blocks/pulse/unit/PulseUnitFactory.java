...
public class PulseUnitFactory extends PulseUnitBlock{
    public int[] capacities = {};

    public Seq<UnitPlan> plans = new Seq<>(4);

    public PulseUnitFactory(String name){
        super(name);
        update = true;
        hasItems = true;
        solid = true;
        ...
        config(Integer.class, (PulseUnitFactoryBuild tile, Integer i) -> {
            tile.currentPlan = i < 0 || i >= plans.size ? -1 : i;
            tile.progress = 0;
        });

        config(UnitType.class, (PulseUnitFactoryBuild tile, UnitType val) -> {
            tile.currentPlan = plans.indexOf(p -> p.unit == val);
            tile.progress = 0;
        });

        consumes.add(new ConsumeItemDynamic((PulseUnitFactoryBuild e) -> e.currentPlan != -1 ? plans.get(e.currentPlan).requirements : ItemStack.empty));
    }
    ...
    public void setBars(){
        super.setBars();
        bars.add("progress", (PulseUnitFactoryBuild e) -> new Bar("bar.progress", Pal.ammo, e::fraction));

        bars.add("units", (PulseUnitFactoryBuild e) ->
            new Bar(
    ...
    public class PulseUnitFactoryBuild extends PulseUnitBuild{
        ...
        public void updateTile(){
            ...
            if(currentPlan != -1 && payload == null){
                ...
                if(progress >= plan.time && allConsValid()){
                    progress %= 1f;
        ...
        public void read(Reads read, byte revision){
            ...
            progress = read.f();
            currentPlan = read.s();
            if(currentPlan > plans.size) currentPlan = plans.size - 1;
        }
    }
}
